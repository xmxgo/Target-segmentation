...
class Unet(object):
    ...
    def detect_image(self, image, count=False, name_classes=None):
        ...
        # 将预测结果转换为8位单通道图像，因为findContours需要8位图像
        pr_uint8 = pr.astype(np.uint8)

        # 寻找轮廓
        _, contours, hierarchy = cv2.findContours(pr_uint8, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        #contours, _ = cv2.findContours(pr_uint8, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

        # 将原始图像转换为OpenCV图像格式以绘制轮廓
        old_img_cv2 = np.array(old_img)

        # 在原始图像上绘制轮廓
        cv2.drawContours(old_img_cv2, contours, -1, contour_color, 2)  # 线宽为2

        # 将修改后的OpenCV图像转换回PIL图像格式
        image_with_contours = Image.fromarray(old_img_cv2)
        ...
        #     seg_img = np.reshape(np.array(self.colors, np.uint8)[np.reshape(pr, [-1])], [orininal_h, orininal_w, -1])
        #     image   = Image.fromarray(np.uint8(seg_img))
        #     image   = Image.blend(Image.fromarray(old_img_cv2), image, 0.7)
